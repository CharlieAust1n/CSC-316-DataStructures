...
public class UpTreeDisjointSetForest<E> implements DisjointSetForest<E> {
	...
	/** Map for storing all the nodes in the DisjointSetForest */
    ...
    private static class UpTreeNode<E> implements Position<E> {
        ...
        /** Number of nodes in a disjoint tree. */
        private int count;
         ...
         * UpTreeNode
         * 
         * @param count Number of nodes in the disjoint tree
         */
    ...
    private UpTreeNode<E> findHelper(UpTreeNode<E> current) { 	
    	UpTreeNode<E> node = validate(current);
    	if(node != node.getParent()) {
    		node.setParent(findHelper(node.getParent()));
    	}
