...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
    	...
    	ALEdge edge = validate(super.insertEdge(origin, destination, edgeData));
    	origin.getOutgoing().addLast(edge);
    	edge.setOutgoingListPosition(origin.getOutgoing().last());
    	destination.getIncoming().addLast(edge);
    	edge.setIncomingListPosition(origin.getIncoming().last());
    	return edge;
    }
