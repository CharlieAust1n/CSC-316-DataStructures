 ...
 * {@see PriorityQueue} abstract data type. HeapPriorityQueue ensures a O(logn)
 * worst-case runtime for {@see PriorityQueue.insert} and
 * {@see PriorityQueue.deleteMin}. HeapPriorityQueue ensures a O(1) worst-case
 * runtime for {@see PriorityQueue.min}, {@see PriorityQueue.size}, and
...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
     ...
     * 
     * Implementation help from the book: Data Structures and Algorithms in Java, Sixth Edition Michael T. Goodrich,
     * Roberto Tamassia, and Michael H. Goldwasser John Wiley and Sons, 2014
     * 
    ...
    protected void downHeap(int index) {
        ...
        while(hasLeft(j)) {
        	...
        	if(compare(list.get(smallestChildIndex).getKey(), list.get(j).getKey()) >= 0) {
        		break;
        	}
