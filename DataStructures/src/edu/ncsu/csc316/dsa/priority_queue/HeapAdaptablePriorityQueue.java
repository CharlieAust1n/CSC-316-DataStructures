...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public static class AdaptablePQEntry<K, V> extends PQEntry<K, V> {
    	/** Store the information of an index of an entry. */
        private int index;
    ...
    public void remove(Entry<K, V> entry) {
    	...
    	int j = temp.getIndex();
    	if(j == list.size() - 1) {
    		// Entry is at the end of list/heap, so just remove it.
    		list.remove(list.size() - 1);
    	} else {
    		// Move entry to remove to end of list/heap.
    		swap(j, list.size() - 1);
    		// Since the entry is now at the end of list/heap, then it can be removed.
    		list.remove(list.size() - 1);
    		// Sort the new list/heap.
    		bubble(j);
    	}
    }
