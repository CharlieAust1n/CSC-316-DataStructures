...
public class BinarySearchTreeMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/** BalanceableBinaryTree that we be intialized. */
     ...
     * @param compare The custom comparator that the user wishes to use.
     */
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
        	...
        	if((left(grandparent) == parent && left(parent) == node) || 
        			(right(grandparent) == parent && right(parent) == node)) {
        		rotate(parent);
        		return parent;
        	} else {
        		rotate(node);
