...
public class SeparateChainingHashMap<K extends Comparable<K>, V> extends AbstractHashMap<K, V> {
    ...
    // * Roberto Tamassia, and Michael H. Goldwasser John Wiley and Sons, 2014.
    // Specific part of method is the size increment.
    @Override
    public V bucketPut(int hash, K key, V value) {
    	Map<K, V> bucket = table[hash];
    	// If there is no map in the bucket, then create one.
    	if(bucket == null) {
    		table[hash] = new AVLTreeMap<K, V>();
    		bucket = table[hash];
    	}
    	int oldSize = bucket.size();
    	// Insert the new key, value into the map and return what was originally there.
    	V rtn = bucket.put(key, value);
    	// Increment the size of the map. (Size may have not changed).
    	size += (bucket.size() - oldSize);
    	// Return the original value in the tree.
    	return rtn;
    }
    ...
    public V bucketRemove(int hash, K key) {
        Map<K, V> bucket = table[hash];
        // If there is no map in the bucket, then return null.
        if(bucket == null) {
        	return null;
        }
        // Remove the key from the map and store the value.
        V rtn = bucket.remove(key);
        // Decrement the size.
        size--;
